...
export class Genetic<T> {
    ...
    public async estimate() {
        ...
        const fitnessPromises = this.entities
          .map((entity) => () => fitnessFunction(entity))
          .map(fn => {
        ...
        const fitnesses = await fitnessPromises.reduce((chain, fn) => {
            return chain.then(fn);
        }, Promise.resolve([] as number[]));


        this.population = (fitnesses as number[])
